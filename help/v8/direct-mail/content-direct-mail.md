...
>abstract="Configurez les propriétés du fichier d’extraction, telles que son nom et son format. Vous pouvez personnaliser le nom du fichier à l’aide d’attributs de la base de données à l’aide de l’éditeur d’expression."

>[!CONTEXTUALHELP]
>id="acw_directmail_properties_content"
>title="Contenu"
>abstract="Dans cette section, indiquez les colonnes à afficher dans le fichier d’extraction. Une fois cette opération terminée, vous pouvez prévisualiser le fichier d’extraction à l’aide du bouton **Simuler du contenu**."

Pour concevoir le contenu du fichier d’extraction généré par votre diffusion courrier, cliquez sur le bouton **[!UICONTROL Modifier le contenu]** de votre page de diffusion, puis configurez les propriétés et le contenu du fichier.
...
1. Dans le champ **[!UICONTROL Format du fichier]**, choisissez le format du fichier d’extraction : **Texte**, **Texte utilisant des colonnes à largeur fixe**, **CSV (Excel)** ou **XML**.
...
+++ Options de format d’extraction disponibles
   ...
   * **[!UICONTROL Séparateur de colonnes]** (format texte) : spécifiez le caractère à utiliser comme séparateur de colonnes dans le fichier d’extraction.
...
1. Une fois la colonne ajoutée, vous pouvez modifier son libellé et modifier l’attribut associé à l’aide de l’icône d’édition.
