...
description: Découvrir comment générer des images à l’aide de l’IA
...
# Générer des images {#generative-image}
...
Utilisez l’IA dans Adobe Campaign Web pour créer un contenu visuel attrayant qui améliore vos messages dans les e-mails, les pages de destination et les notifications push. L’IA vous aide à générer et à optimiser des images, afin de garantir que votre contenu soit visuellement attrayant et en accord avec votre marque.

## Pour les e-mails et les pages de destination {#email-web-channels}

Utilisez l’IA pour générer des expériences visuelles complètes pour vos diffusions par e-mail et vos pages de destination. Cette fonctionnalité vous permet de produire des images attrayantes et conformes à votre marque qui trouvent écho auprès de votre audience sur l’ensemble des points de contact numériques.

### Accéder et configurer {#access-configure}

Pour commencer à générer des images avec l’IA, configurez d’abord votre diffusion et ouvrez l’éditeur de contenu. Suivez les étapes ci-dessous pour préparer votre espace de travail et accéder au panneau Générer du contenu.
...
1. Sélectionnez la ressource à modifier et accédez au menu **[!UICONTROL Générer du contenu]**.
...
### Générer une image {#generate-content}

Découvrez comment créer des prompts efficaces et configurer les paramètres d’image pour générer des images visuellement attrayantes avec l’IA. Personnalisez des paramètres tels que le rapport L/H, l’intensité visuelle et l’éclairage pour créer des images conformes à votre marque et à vos objectifs de campagne.

1. Activez l’option **[!UICONTROL Style de référence]** pour utiliser l’IA afin de personnaliser le nouveau contenu en fonction du contenu sélectionné.
...
1. Adaptez votre prompt à l’aide de l’option **[!UICONTROL Paramètres d’image]** :
   ...
   * **[!UICONTROL Modèle génératif]** : faites votre choix parmi le **[!UICONTROL modèle Adobe]** prêt à l’emploi, le **[!UICONTROL modèle de partenaire]** pour les fonctionnalités spécialisées ou les **[!UICONTROL modèles personnalisés]** formés sur les ressources de votre marque. [En savoir plus](generative-models.md)
   * **[!UICONTROL Format]** : déterminez la largeur et la hauteur de la ressource. Choisissez parmi les formats courants tels que 16:9, 4:3, 3:2 ou 1:1, ou saisissez une taille personnalisée.
...
Utilisez l’IA pour générer des images attrayantes pour les notifications push, afin de créer des communications mobiles visuellement attrayantes qui captent l’attention et trouvent écho auprès de votre audience.

### Accéder et configurer {#mobile-access-configure}

Pour commencer à générer des images pour les notifications push avec l’IA, configurez d’abord votre diffusion et ouvrez l’outil Générer du contenu.
...
1. Accédez au menu **[!UICONTROL Afficher Générer du contenu]**.
   ...
   ![Copie d’écran affichant le menu Afficher Générer du contenu](assets/push-img-1.png){zoomable="yes"}

### Générer une image {#mobile-generate-content}

Après avoir accédé à Générer du contenu, vous pouvez ajuster les paramètres de génération afin de créer des images qui correspondent à votre marque et répondent aux objectifs de vos notifications push. Configurez les paramètres de prompt et d’image pour générer des visuels optimisés pour les affichages mobiles.
...
1. Ajustez le contenu en décrivant ce que vous souhaitez générer dans le champ **[!UICONTROL Prompt]**.
   ...
   ![Générer du contenu avec le champ et les options de prompt](assets/push-img-2.png){zoomable="yes"}
