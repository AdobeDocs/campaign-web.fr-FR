...
Une fois que vous avez [créé un workflow](create-workflow.md), à partir du menu de workflow ou au sein d’une campagne, vous pouvez commencer à orchestrer les différentes tâches qu’il effectue. Pour ce faire, une zone de travail visuelle dédiée vous permet de créer un diagramme de workflow. Dans ce diagramme, vous pouvez ajouter différentes activités et les enchaîner dans un ordre séquentiel.
...
* **Afficher la carte** : ouvrez un instantané de la zone de travail indiquant votre emplacement.
...
* **Désactivez/activez** l’activité. Lorsque le workflow s’exécute, les activités désactivées et les activités suivantes qui se trouvent sur le même chemin ne sont pas exécutées et le workflow s’arrête.
* **Mettez en pause/Reprenez** l’activité. Lorsque le workflow s’exécute, il se met en pause au niveau de l’activité en pause. La tâche correspondante, ainsi que toutes celles qui lui succèdent dans le même chemin, ne sont pas exécutées.
...
* Copiez plusieurs activités à l’aide du bouton de la barre d’outils.

  ![bouton Copier plusieurs activités](assets/workflow-copy-2.png){zoomable="yes"}{width="70%"}

Pour coller les activités copiées, cliquez sur le bouton **+** d’une transition et sélectionnez « Coller l’activité X ».
...
Journey Optimizer vous permet de déplacer une activité, ainsi que l’ensemble du contenu de ses nœuds enfants (y compris toutes les transitions et activités qu’elle contient), vers la fin d’une autre transition dans le même workflow.
...
1. Sélectionnez l’activité à déplacer.
...
Le champ **Exécution** permet de définir l&#39;action à effectuer au démarrage de la tâche.

Le champ **Durée d’exécution maximale** vous permet de spécifier une durée telle que « 30 s » ou « 1 h ». Si l’activité n’est pas terminée après la durée spécifiée, une alerte est déclenchée. ce qui n’a par ailleurs aucun impact sur le fonctionnement du workflow.

Le champ **Fuseau horaire** vous permet de sélectionner le fuseau horaire de l’activité. Adobe Campaign gère les décalages horaires entre plusieurs pays sur la même instance. La configuration appliquée est paramétrée lors de la création de l’instance.

Le champ **Affinité** vous permet de forcer l’exécution d’un workflow ou d’une activité de workflow sur une machine en particulier. Pour ce faire, spécifiez une ou plusieurs affinités pour le workflow ou l’activité en question.

Le champ **Comportement** vous permet de définir la marche à suivre en cas de tâches asynchrones.

### Gestion des erreurs

Le champ **En cas d’erreur** permet d’indiquer l’action à effectuer si l’activité rencontre une erreur.
...
Pour ce faire, les activités suivantes ont été ajoutées :
...
* des activités **[!UICONTROL Créer une audience]** destinées à cibler les trois ensembles de clients et clientes :
   ...
   * Les clients et clientes appartenant à l’audience préexistante « VIP to reward »
...
Une fois le workflow terminé, ajoutez une activité **[!UICONTROL Fin]** à la fin du diagramme. Cette activité marque visuellement la fin d’un workflow et n’a aucun impact fonctionnel.

Une fois le diagramme de workflow conçu, exécutez le workflow et suivez l’avancement de ses différentes tâches. [Découvrez comment démarrer un workflow et surveiller son exécution](start-monitor-workflows.md).