...
>abstract="L’activité **Code JavaScript** permet d’exécuter un script JavaScript dans le contexte d’un workflow. Vous pouvez ainsi effectuer des actions ou collecter des informations à partir de la base de données. Utilisez des activités de code **Simple** JavaScript pour exécuter un fragment de code lors de l’exécution du workflow. **Avancé** les activités de code JavaScript vous permettent d’effectuer des opérations plus complexes en exécutant séquentiellement deux fragments de code différents. La première fois que le workflow démarre, le premier appel est exécuté. Chaque fois que le workflow s’exécute de nouveau, le code défini dans le deuxième appel est exécuté."

>[!CONTEXTUALHELP]
>id="acw_orchestration_javascript_snippet"
>title="Extrait JavaScript"
>abstract="Définissez le script à exécuter lors de l’exécution de l’activité. Si vous configurez une activité de JavaScript **Avancée**, modifiez deux fragments de code : le premier code d’appel à exécuter lors de la première exécution du workflow et le code d’appel suivant à exécuter lors des exécutions suivantes du workflow."
...
L’activité **Code JavaScript** est une activité de **Gestion des données**. Utilisez cette activité pour exécuter un script JavaScript dans le contexte d’un workflow. Vous pouvez ainsi collecter des informations dans la base de données ou effectuer d’autres opérations complexes.
...
1. Choisissez le type d&#39;activité à créer :
   ...
   * **Avancé** : effectuez des opérations plus avancées en exécutant deux fragments de code différents. [Découvrir comment configurer une activité JavaScript avancée](#advanced)

   >[!NOTE]
   >
   >Avec l’interface utilisateur web de Campaign, deux activités ont été consolidées en une seule en fusionnant les fonctionnalités de code JavaScript **simple** et **avancé**. Cette consolidation n’a aucune incidence sur la fonctionnalité de l’activité.
...
* Exécuter deux extraits de code différents. Le premier fragment de code est exécuté au premier démarrage du workflow. Chaque fois que le workflow s’exécute de nouveau, l’extrait de code défini dans le deuxième appel est exécuté.
...
1. Sélectionnez le type **Avancé**, puis configurez les fragments de code à exécuter :
...
1. Pour ajouter une ou plusieurs transitions de sortie, cliquez sur le bouton **[!UICONTROL Ajouter des transitions]** et indiquez un libellé et un nom interne pour chaque transition.
   ...
   Dans cet exemple, deux transitions sont configurées et activées par le script dans le fragment de code, selon des conditions spécifiques.

   ![Exemple de transitions configurées](../assets/javascript-transitions.png)

1. Terminez la configuration de l’activité et démarrez le workflow.
...
1. Cliquez sur **Modifier le code** et configurez le fragment de code comme illustré ci-dessous :
...
Dans cet exemple, le workflow commence par une activité **Signal externe** dans laquelle une variable (`interest`) est transmise par un autre workflow. La valeur de la variable est « Course » ou « Yoga », selon les opérations de filtrage effectuées dans le workflow initial.
...
1. Cliquez sur **Modifier le code du premier appel** et configurez le fragment de code comme illustré ci-dessous :
...
1. Effectuez le paramétrage de chaque transition en fonction de vos besoins, puis démarrez le workflow. L&#39;une des deux transitions de sortie est activée en fonction de la valeur de la variable `interest` passée par l&#39;activité **Signal externe**.